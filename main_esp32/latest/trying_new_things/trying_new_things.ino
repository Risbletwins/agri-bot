...
void seedSowTask(void *pvParameters) {
  static int prev_seed_sow_state = 0;

  for (;;) {
    if (seed_sow_state != prev_seed_sow_state) {
      if (seed_sow_state == 1) {
        Serial.println("seed sow has started.");
      } else {
        Serial.println("seed sow has ended.");
      }
      prev_seed_sow_state = seed_sow_state;
    }
...
void soilMoistureTask(void *pvParameters) {
  static int prev_soil_moisture_state = 0;

  for (;;) {
    if (soil_moisture_state != prev_soil_moisture_state) {
  ...
  for (;;) {
    if (WiFi.status() != WL_CONNECTED) {
      ...
      if (httpResponseCode > 0) {
        ...
        String new_response = http.getString();
